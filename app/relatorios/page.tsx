...
export default function PlanilhaPage() {
  ...
  const uniqueResponsaveis = Array.from(
    new Set(
      tasks
        ...
        .map(task => task.responsavel_email as string)
    )
  )
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    return matchStatus && matchResponsavel && matchPrioridade && matchDataInicio && matchDataFim
  })
  ...
  const inProgressTasks = filteredTasks.filter(task => getStatusName(task.status_id) === "Em desenvolvimento").length
      ...
      <div className="p-4 sm:p-6 md:p-10">
        ...
        <div className="flex flex-col mb-4">
          <div className="space-y-1">
            <h1 className="text-2xl sm:text-3xl font-bold">Planilha</h1>
            <p className="text-sm text-muted-foreground">Lista completa de todas as atividades do sistema</p>
          </div>
        </div>
        ...
        <div className="mb-6">
          <TasksByResponsavelChart tasks={tasks} />
        </div>
